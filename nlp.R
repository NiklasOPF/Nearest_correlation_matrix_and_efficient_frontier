...
#stock.close <- as.xts(merge(MSFT, AAPL, AMZN, GOOGL,IBM))[,c(4,10,16,22,28)]
#View(stock.close)

#plot(stock.close$AMZN.Close)
#lines(stock.close$MSFT.Close, col="blue")
#lines(stock.close$AAPL.Close, col="red")
#lines(stock.close$GOOGL.Close, col="green")


#stockcor<- cor(stock.close)
#stockcor[1,2] <- -1 * stockcor[1,2]
#stockcor[2,1] <- stockcor[1,2]
#out = nearcorr(stockcor)
#new <- nearPD(stockcor)
#difference between nearPD and nearCOR

setwd("C:/Users/e0352757/Desktop")
d<-read.csv("newdata.csv",header = FALSE)
nd<-d
#d[, -c(1)]
c= matrix(0, nrow=1516, ncol=1516)
c<- cor(nd, use="pairwise.complete.obs", method= "pearson")
c[is.na(c)] <- 0
e<- eigen(c)
range(e$values)
sum(e$values<0)
...
a<- CorrelationMatrix(G,b,tau=0,tol=1e-6)
res<- a$CorrMat
val<- eigen(res)
range(val$values)
sum(val$values >=0)
